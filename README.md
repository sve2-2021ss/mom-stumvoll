...
The following image gives a basic overview of the systems components and how they interact with each other. How these components work will be described in this section.
...
The `DeviceWatcher` is the bridge between the `SystemValueSources` and the `SystemValueApi`. He is registered to all `SystemValueSources` and forwards the messages to the `SystemValueApi`. As already mentioned in [SystemValueSource](#systemvaluesource) he also attaches the device identifier to the `SystemValues`. He additionally decides which messages should be acknowledged (more on that in [Confirmation](#confirmation)).
...
Additionally the `protobuf-net.BuildTools` library was used, which helps catch errors in `proto` contracts (e.g. multiple members with the same tag) at compile time.
...
The following sections describe the aspects of the communication between the `Agent` and the broker.
...
Before describing the `Agent` implementation the basics of RabbitMQ confirmations are explained first.

###### Basics

When sending messages to a RabbitMQ broker it is possible to request a response if the broker accepted or declined a message. For this the channel on which the message is sent must be put in the confirm mode. When this is done, both the publisher and the broker will count the messages sent over this channel starting from 1. When the broker acks or nacks a message he will send the number of the message back to the producer on the same channel. The broker can additionally send a `multiple` flag which indicates that all messages till the given number have been handled or declined.

###### Implementation

To implement this acknowledgement behaviour the `SystemValueApi` needs two separate channels, one in confirm mode and one in the default mode. To put a channel in confirm mode the `ConfirmSelect` method needs to be called on the channel object. To react to the brokers response the C# RabbitMQ library exposes two events `BasicAcks` and `BasicNacks`. Both contain the message number as well as the multiple flag.

Every time the `SystemValueApi` send a message which needs to get confirmed it stores this message in a `ConcurrentDictionary` `_ackMap`. The key is the message number and the value is the `SystemValue`. The message number of the next message can be requested by calling `NextPublishSeqNo` on the channel object. Whenever `BasicAcks` or `BasicNacks` fires an event the message in question will be removed from the dictionary. In case the message was nacked it will be resent.
...
RabbitMQ is really great for implementing asynchronous communication. It's very easy to get started as a minmal running system can be set up quickly using docker.
There are a lot of client libraries which makes its very easy to communicate between different programming languages and frameworks.
...
The communication through a topic exchange is very flexible as the consumer can decide for himself in which messages he is interesed by binding a queue to the desired routing keys.
Routing keys allow the usage of wildcards however only two, `*` (substitute one word) and `#` (substitute zero or more words), are supported. Regex patterns like `device-(1|4)` are not possible, although this could be mimicked by binding a queue to multiple routing keys. Another issue is that there is no way for the consumers to "discover" the actively used routing keys. There is also no way for consumers to know the contents of each message since there is no "API contract".
